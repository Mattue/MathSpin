...
Void Stage_obj::__onKey( Dynamic event,::String type){
{
		...
		if (((stack->length > (int)0))){
			...
			if ((keyboardEvent->__getIsCancelled())){
				...
				HX_STACK_LINE(836)
				if (((bool((bool((bool((((int(flags) & int(::openfl::_v2::display::Stage_obj::efCtrlDown))) > (int)0)) && bool((((int(flags) & int(::openfl::_v2::display::Stage_obj::efCommandDown))) > (int)0)))) && bool((type == ::openfl::_v2::events::KeyboardEvent_obj::KEY_DOWN)))) && bool((event->__Field(HX_CSTRING("value"),true) == (int)70))))){
					HX_STACK_LINE(839)
